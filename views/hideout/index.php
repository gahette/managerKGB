...
<div class="row">
    <?php foreach ($params['hideouts'] as $hideout): ?>
        <div class="col-md-4">
            <div class="card mb-3">
                <div class="card-body">
                    <h2><?= $hideout->getCode() ?></h2>
                    <h3><?= $hideout->getAddress() ?></h3>
                    <div>
                        ...
                        <?php foreach ($hideout->getTypes() as $type): ?>
                            <p class="badge bg-success"><a href="/managerKGB/country/<?= $type->getId() ?>"
                                                           class="text-white link-underline link-underline-opacity-0"><?= $type->name ?></a>
                            </p>
                        <?php endforeach; ?>
                    </div>
                    <a href="/managerKGB/agent/<?= $hideout->getId() ?>" class="btn btn-dark">Lire plus</a>
                </div>
            </div>
        </div>
    <?php endforeach; ?>
